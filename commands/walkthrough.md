---
allowed-tools: Bash(artifact-read.sh:*), Bash(artifact-patch.py:*), Bash(artifact-validate.sh:*), Bash(artifact-render.py:*), Bash(artifact-publish.sh:*), Bash(repo-slug.sh:*), Bash(log-tokens.sh:*), Bash(tally-subagent-tokens.sh:*), Bash(orchestrator-tokens.sh:*), Bash(git:*), Bash(jq:*), Bash(date:*), Bash(cat:*), Bash(printf:*), Bash(mkdir:*), Bash(mv:*), Bash(rm:*), Bash(tr:*), Bash(awk:*), Bash(mktemp:*), Agent, Read, AskUserQuestion
argument-hint: "[threshold]"
description: Walk interactively through findings /jds:fix would skip, above a score floor. Per-finding briefing + options + recommendation, then batch re-render and write a local decisions log. Never writes to GitHub.
disable-model-invocation: false
---

Walk the reviewer through findings in the latest `/jds:review`
artifact that `/jds:fix` would **skip** — deep-manual,
...
the shared `promote-core.md` fragment with `--defer-render`
semantics) or a skip. At the end, render the updated review once,
then write a decisions-log document into the review directory for
audit. Nothing is written to GitHub.
...
- `[threshold]` (optional, positional) — non-negative integer score
  ...
  low-signal findings. Independent of the `/jds:fix`
  threshold — promoted findings are picked up by `/jds:fix`
...
5. For each remaining finding in the chosen tier:
   ...
   - Dispatches a promote (patch + trace, no render) or a skip.
6. Re-renders `artifact.md` once.
7. Appends the local publish trace entry.
8. For each `pre_existing_report` finding: offers to draft a
   follow-up note, one by one, appended to
   `$review_dir/follow-ups.md` (a local stand-in for issue filing).
9. Appends a "Walkthrough decisions" entry to
   `$review_dir/walkthrough-decisions.md` with the full log of what
   was auto-accepted / promoted / skipped / follow-ups drafted / why.
10. Appends a `## walkthrough (<ts>)` block to `trace.md`.
11. Prints a user-visible summary.

Does NOT run `/jds:fix`. Does NOT surface `disposition=disproven`
findings (those require an explicit `/jds:promote <id> --force`).
...
`/jds:fix` default and the Phase 4 moderate-confirmation
...
(Artifacts produced by `/jds:review` are always `mode=local`; this
command needs no mode or PR context — nothing is published anywhere.)

### 3. Compute walkthrough scope

The walkthrough surfaces findings `/jds:fix` would SKIP at
...
Either every finding is already auto-eligible (run /jds:fix
...
If `scope_full_count == 0` but `scope_preexisting_count > 0`, fall
through to step 4 anyway — the preflight will note that the walk has
no scope but the follow-up-drafting phase (§6.5) still has work to
offer.
...
- **Phase 8 fix gate (default 60)** — what `/jds:fix` touches:
...
- **Walkthrough score floor (`$threshold`, default 60)** — the
  ...
  `/jds:fix` regardless of its threshold, via the
...
Phase 5.5 of `/jds:review` (and `:add`)
...
mutations at all from this walk), there's nothing to re-render. Skip
steps 6.1 and 6.2; proceed to step 6.5 (follow-up drafting) and then
step 7 (decisions log). Follow-up drafting is independent of walk
activity — a reviewer who skipped every walk finding may still want
to draft follow-ups for pre-existing findings. The scope filter +
preview table already showed the user what's in the backlog; a
decisions log with "skipped all 10" is still useful audit.
...
so their cost will only surface on the next `/jds:fix` (or
subsequent `/jds:add` / `/jds:walkthrough`) run when
that command re-tallies. This is acceptable — issue-filing is terminal
and no further re-publish follows it.

#### 6.2. Local publish trace entry
...
No-op that appends a one-line trace entry. Continue to step 6.5.

### 6.5. Draft follow-up notes for pre-existing findings (local)

Skip entirely when `scope_preexisting_count == 0` — nothing to draft.

Rationale: `pre_existing_report` findings originate in code that
pre-dates this change (per the §13 pre-existing override). They're
not fixable in the current change, so bundling them with
promote-eligible findings in the §5 loop forced awkward "skip"
decisions every run. Drafting a follow-up note moves them to a
durable local document (`$review_dir/follow-ups.md`) — ready to be
pasted into whatever tracker you use; nothing is filed on GitHub.

Initialize an `issues_filed=[]` array in ambient context. Each entry:
`{finding_id, issue_url, title, ts}` — `issue_url` holds the local
document path (kept under the upstream field name so §7.1/§8/§9
render unchanged).

```bash
followups_path="$review_dir/follow-ups.md"
```
...
- "Draft all $scope_preexisting_count follow-up note(s)"
- "Pick a subset"
- "Skip — don't draft any"
...
§9 summary's "not drafted" note will fire.

#### 6.5.2. Per-selected finding: draft + confirm + save
...
lists its id. Drafting a follow-up for a finding that's also queued
...
records what was actually drafted, not what was considered.

**Dispatch a Sonnet drafting agent.** Model: `sonnet`. Budget: ~2-3k
tokens. Prompt:

> You are drafting a follow-up note (issue-shaped) for a code-review
> finding that pre-dates the current change (`pre_existing_report`).
> The reviewer wants a concise, actionable write-up that someone
> triaging the backlog can read cold — it may later be pasted into an
> issue tracker verbatim.
>
> Return strict JSON matching:
>
> ```json
...
>     and a "Discovered during" sentence naming this review
>     (see context).
>   - Keep the body under 40 lines. Don't replicate the finding JSON.
>
> Context:
>
>   - Finding JSON: <paste $finding_json verbatim>
>   - Review: `$review_id` on branch `$head_branch`
...
```markdown
## Pre-existing follow-up draft — $finding_id
...
**For "Save follow-up note":**

```bash
{
    printf '\n---\n\n'
    printf '## %s — %s\n\n' "$finding_id" "$draft_title"
    printf '_drafted %s · review %s · branch %s_\n\n' \
        "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$review_id" "$head_branch"
    # append $draft_body (possibly user-edited)
    cat <<'BODY'
<draft body here>
BODY
} >> "$followups_path"
issue_url="$followups_path#$finding_id"
```
...
Print a one-line chat update: "Drafted $finding_id → $followups_path.
Ns processed of M." Mirrors the §5.6 cadence for the main loop.

### 7. Write the decisions log (local document)

Skip when `${#decisions[@]} == 0` AND `${#issues_filed[@]} == 0`
— there's nothing to report (the reviewer cancelled before walking
anything AND drafted no follow-ups). Step 8's trace line is still
written so the run is auditable.

Render a decisions-log markdown block from `decisions` and APPEND it
to `$review_dir/walkthrough-decisions.md` — an append-only local
audit document; each `/jds:walkthrough` run adds a fresh entry.
...
Promoted findings will be picked up by the next `/jds:fix` run via the `human_confirmation` bypass, regardless of its score threshold.
...
- **F023** — [first line of claim]
  ...
  - Resume with `/jds:walkthrough $threshold`.

#### Pre-existing follow-ups drafted

- **F026** — <draft title> → <local path#anchor>
- **F028** — <draft title> → <local path#anchor>

---

Decisions log: this document is append-only audit — entries are never
edited in place. Each `/jds:walkthrough` run appends a fresh entry.
Current state: see `artifact.md` alongside this file.
```
...
skips the write only when BOTH decisions and issues_filed are empty).
...
in the rendered entry matches the order the actions occurred.

Emit the Auto-accepted subsection only when `auto_accept_count > 0`,
the Pre-existing follow-ups subsection only when `issues_filed[]` is
...
by the next `/jds:fix`…" sentence entirely when
...
#### 7.2. Append to the local decisions log

```bash
comment_body_path=$(mktemp -t jds-walkthrough-body.XXXXXX)
# ... write the rendered markdown to $comment_body_path ...

decisions_log_path="$review_dir/walkthrough-decisions.md"
cat "$comment_body_path" >> "$decisions_log_path"
printf '\n\n' >> "$decisions_log_path"
rm -f "$comment_body_path"
```

Capture `decisions_log_path` for the §8 trace entry and the §9
summary. Tell the user in one line where the decisions log landed.
...
```bash
{
    ...
    [[ -n "${decisions_log_path:-}" ]] && \
        printf 'decisions_log=%s\n' "$decisions_log_path"
    printf '\n'
} >> "$trace_log_path"
```
...
Pre-existing follow-ups drafted: <N; list id → path pairs below, or omit section>
  F026 → <path#anchor>
  F028 → <path#anchor>
...
Decisions log: $decisions_log_path
Rendered report: $review_dir/artifact.md (updated in place).

You can resume later by re-running /jds:walkthrough — the
...
enable raw mode). Note that §6.5 follow-up-drafter agents dispatched after
...
common-case summary tidy. Omit the "Pre-existing follow-ups drafted"
section entirely when `issues_filed[]` is empty.

When `scope_preexisting_count > 0` AND `${#issues_filed[@]} == 0`
(the reviewer saw pre-existing findings but chose "Skip — don't
draft any"), add a one-line note after the summary block:

```
Note: $scope_preexisting_count pre-existing finding(s) without a follow-up note.
Re-run /jds:walkthrough to revisit.
```
...
pattern as `/jds:promote` step 10).

## What this command does NOT do

- **No `disposition=disproven` handling.** Disproven findings need
  `/jds:promote <id> --force` with a conscious justification;
