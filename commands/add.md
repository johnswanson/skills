...
description: Inject externally-sourced findings (cloud reviews, manual finds, etc.) into the most recent /jds:review artifact for this branch. Validates via Phase 4 and re-renders locally. Never writes to GitHub.
...
`/jds:review` (which overwrites the artifact).
...
Then resolve the deep-lane model — used by step 7.3's per-candidate
validators:
...
- Anything else (e.g. only `cli_file` set, or all empty) → error-as-prompt:
  ...
  >   1. Free-form paste:    `/jds:add <paste body...>`
  >   2. Structured one-shot: `/jds:add --file <path> --line <N> --claim "<one-sentence claim>"`
...
> ERROR: previous /jds:fix run did not finish (N findings
...
- **(a) Stop — I'll merge `$merge_ref` into `$head_branch` first, then re-run.** Emit a `branch_behind_base stopped` trace line, then exit 0 with: `Stopping. Run \`git merge $merge_ref\` (or fast-forward) on \`$head_branch\`, then re-run /jds:add.`
  ```bash
...
src_tmp=$(mktemp -t jds-add-srcs.XXXXXX)
...
STILL run step 8 (re-render) and step 9 (local publish trace) — the artifact
...
`/jds:review` Phase 0, `/jds:add` has no clean-tree gate.
...
#### 7.3 Deep-lane dispatch (`$high_model`, one sub-agent per candidate)

For each id in `deep_ids`, launch ONE `Agent` tool-use with `model:
$high_model` (resolved at step 1), `subagent_type: general-purpose`,
dispatched in a single orchestrator turn for concurrency. Read the
finding JSON and pass it inline.

**One `$high_model` per candidate — never collapse multiple deep-lane
candidates into a single batched call.** §7.6's `--apply-decisions --expected
...
Prompt essence — kept self-contained here; do NOT dispatch through `fragments/05-validation.md` (it pulls in Wave-2 / §4.6 / §4.4.5 logic that doesn't apply to the `/jds:add` path).
...
> 7. This finding was injected by `/jds:add` from an external
...
`phase_add`, agent_role `validator`, `--finding-id` set, model
`$high_model`).
...
user work. `/jds:add` has no Phase-0 dirty-tree gate, so this
...
```bash
scratch="/tmp/jds-review-add-$review_id"
mkdir -p "$scratch"
...
# Used by --expected as the structural guard against batched-deep-lane
...
deep-lane dispatches were collapsed (re-dispatch one Agent per missing
...
spend on top of the prior `/jds:review` baseline. The paste
normalizer (§4b) and any Phase-4 re-validators that ran during this
`/jds:add` invocation already logged their sub-agent usage to
...
Next:
  - /jds:fix             apply auto-eligible findings (deep confirmed_mechanical) plus auto-recommendations via Phase 7.5 batch-accept
  - /jds:walkthrough     review any remaining non-promoted findings (light-lane / manual / uncertain)
```
...
- **No persistence across fresh `/jds:review` runs.** A re-review
...
> jds-review candidate schema. The reviewer has either pasted a chat
> transcript / review summary from another tool (Claude Code
> /ultrareview, a Claude chat, CodeRabbit text output, a teammate's Slack
