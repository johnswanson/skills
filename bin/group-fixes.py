...
def _check_eligible(finding, fid):
    ...
    if not isinstance(vr, dict):
        ...
        c.err_prompt(
            ...
            action="re-run /jds:review to refresh validation_result, or exclude this finding from --eligible-finding-ids. (Promoted findings with null validation_result are accepted — this one is not promoted.)"
        )
        sys.exit(c.EXIT_VALIDATION)
    ...
    if not isinstance(ftm, list):
        c.err_prompt(
            ...
            action="schema requires this field — re-run /jds:review if the artifact is malformed."
        )
        sys.exit(c.EXIT_VALIDATION)
...
def _compute_groups(artifact, eligible_ids):
    ...
    if unknown:
        ...
        c.err_prompt(
            ...
            action="re-run /jds:review or fix the orchestrator's eligibility filter."
        )
        sys.exit(c.EXIT_VALIDATION)
...
def main():
    ...
    try:
        ...
        c.err_prompt(
            ...
            action="check --artifact; run `/jds:review` if this branch has no review."
        )
        return c.EXIT_VALIDATION
    except json.JSONDecodeError as e:
        c.err_prompt(
            f"artifact at {args.artifact} is not valid JSON: {e.msg} (line {e.lineno}, col {e.colno})",
            action="the on-disk file is corrupted — restore from git or re-run /jds:review."
        )
        return c.EXIT_VALIDATION
    ...
    if errors:
        ...
        c.err_prompt(
            ...
            action="re-run /jds:review to regenerate a valid artifact."
        )
        return c.EXIT_VALIDATION
