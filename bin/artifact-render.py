...
def render_header(artifact):
    ...
    if not any_fix_attempts(artifact):
        lines.append("**Fix threshold:** not yet set (run `/jds:fix [threshold]` to apply fixes)")
    ...
    # JDS_REVIEW_TALLY_ORCHESTRATOR opt-in landed). Zero-turn means
...
def render_auto_recommendations(buckets):
    ...
    lines.append(
        ...
        "Run `/jds:fix` to batch-apply with one confirmation, or "
        "`/jds:walkthrough` to review one-by-one._"
    )
    lines.append("")
...
def render_deep_other(buckets, disposition):
    ...
    if disposition == "confirmed_manual":
        lines.append(
            "_Not auto-applied by `/jds:fix` directly — these need a confirmation step. "
            "Findings with an auto-recommendation get batch-confirmed at `:fix`'s Phase 7.5 "
            "preflight (or `:walkthrough` Step 4.5); use `/jds:promote <finding_id>` "
    ...
    if disposition == "uncertain":
        lines.append("")
        lines.append("Phase 4 couldn't confirm decisively. Re-run `/jds:review` if you suspect this deserves")
...
def render_light_lane(buckets):
    ...
    if any(f.get("disposition") in ("confirmed_mechanical", "confirmed_manual") for f in rows):
        lines.append(
            ...
            "`/jds:fix` directly. Findings with an auto-recommendation get "
            "batch-confirmed at `:fix`'s Phase 7.5 preflight (or `:walkthrough` Step 4.5); "
            "use `/jds:promote <finding_id>` for a single-finding manual override._"
        )
        lines.append("")
...
def render_footer(artifact):
    return "🤖 Generated with the jds review plugin (adapted from [adamsreview](https://github.com/adamjgmiller/adamsreview))"
