...
#           `unsafe: files <list> changed since review; re-run /jds:review or use --force`.
#           exit 1.
#
...
if ! REVIEWED_RESOLVED=$(git rev-parse --verify "${REVIEWED_SHA}^{commit}" 2>/dev/null); then
    ...
    echo "Action: re-run /jds:review to regenerate the review against the current history." >&2
    exit 1
fi
...
if ! git merge-base --is-ancestor "$REVIEWED_RESOLVED" HEAD 2>/dev/null; then
    ...
    echo "Action: re-run /jds:review against the current branch." >&2
    exit 1
fi
...
echo "unsafe: files $COMMA_LIST changed since review; re-run /jds:review or use --force" >&2
exit 1
