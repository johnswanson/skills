...
Modes (CLI flags; mutually exclusive):
  ...
  --apply-decisions <array> apply a batch of Phase-4 decision tuples in one
                            ...
                            deep-lane: one deep-model agent per candidate; light-lane:
...
def cmd_add_findings(args):
    ...
    code-aligned spec:
      ...
      - 7       : EXIT_ALL_REJECTED — input was a JSON array, but every
                  ...
                  same; /jds:add migration may want to branch.
...
def cmd_apply_decisions(args):
    ...
    #     single batched deep-lane call (re-dispatch one Agent per candidate);
    ...
    if args.expected > 0 and len(decisions) != args.expected:
        ...
        c.err_prompt(
            ...
            action=(
                ...
                "deep-lane call (re-dispatch one Agent per candidate and recompose "
    ...
    for idx, tup in enumerate(tuples):
        ...
        if before_state != "open":
            ...
            c.err_prompt(
                ...
                valid_values=(sorted(allowed) or ["(none — terminal state)"]),
                action="run /jds:fix fresh; if this persists, check the Phase 8 step 8.1 eligibility filter."
            )
            sys.exit(c.EXIT_INVALID_TRANSITION)
...
# whose auto_fix_hint the user accepted at /jds:fix preflight or
# /jds:walkthrough Step 4.5.
#
...
#                               concurrent /jds:add) or findings
    ...
    if not accepted_ids and rejected:
        c.err_prompt(
            ...
            action=(
                ...
                "concurrent /jds:add; findings already promoted between Phase "
    ...
    for idx, entry in enumerate(entries):
        ...
        if not afh or not isinstance(afh, dict) or not afh.get("hint"):
            c.err_prompt(
                ...
                action="re-run /jds:review (Phase 5.5 generates auto_fix_hint), or use /jds:promote for a manual promote."
            )
            return c.EXIT_VALIDATION
        ...
        if disp not in _AUTO_REC_PROMOTABLE_DISPOSITIONS:
            c.err_prompt(
                ...
                action="auto-rec eligibility (§5.5) requires confirmed_mechanical / confirmed_manual / confirmed_report. Use /jds:promote for other dispositions."
            )
            return c.EXIT_VALIDATION
