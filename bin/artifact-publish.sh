...
#      where <reviews-root> is $JDS_REVIEW_REVIEWS_ROOT (default
#      ~/.jds-reviews). If the resolved review_id disagrees with
...
# Continuation intent is the caller's responsibility: fresh /jds:review
# calls without --comment-id (→ new comment); /jds:fix and
# /jds:promote carry the prior comment_id forward from the
...
resolve_md_path() {
    ...
    if [[ -n "$REPO_SLUG" && -n "$BRANCH" ]]; then
        local reviews_root_base="${JDS_REVIEW_REVIEWS_ROOT:-$HOME/.jds-reviews}"
        ...
        if [[ ! -f "$latest_file" ]]; then
            ...
            echo "Action: run /jds:review against this branch first, or pass --md-path explicitly." >&2
            exit 1
        fi
        local resolved_id
        ...
        if [[ -z "$resolved_id" ]]; then
            ...
            echo "Action: rerun /jds:review to repopulate, or pass --md-path explicitly." >&2
            exit 1
        fi
...
# Step 1: --comment-id → PATCH directly. Caller (e.g. /jds:fix,
# /jds:promote, or Phase 0 step 0.14's opt-in recovery path)
...
# Step 2: no --comment-id → create a new comment. Fresh /jds:review
