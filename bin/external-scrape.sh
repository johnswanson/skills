...
# $JDS_REVIEW_FIXTURES_USER (default $(whoami)).
# Default chain (first found wins):
#   .claude/review-config.json                      (per-repo, cwd-relative)
#   $JDS_REVIEW_CONFIG_ROOT/review-config.json    (override for tests)
#   ~/.jds-reviews/review-config.json             (global)
#
...
resolve_config_path() {
    ...
    local test_root="${JDS_REVIEW_CONFIG_ROOT:-}"
    if [[ -n "$test_root" && -f "$test_root/review-config.json" ]]; then
        echo "$test_root/review-config.json"
        return
    fi
    if [[ -f "$HOME/.jds-reviews/review-config.json" ]]; then
        echo "$HOME/.jds-reviews/review-config.json"
        return
    fi
    echo ""
}
...
WORK=$(mktemp -d -t jds-review-scrape.XXXXXX)
...
if [[ -n "$FIXTURES_DIR" ]]; then
    ...
    CURRENT_USER="${JDS_REVIEW_FIXTURES_USER:-$(whoami)}"
    ...
    if [[ "$FAIL" -ne 0 ]]; then
        ...
        if grep -qi "rate limit" "$ISSUE_ERR" "$REVIEW_ERR" "$RVCMT_ERR" 2>/dev/null; then
            echo "Action: wait for the rate limit to reset (see 'X-RateLimit-Reset' headers above), or re-run /jds:review without --ensemble (which skips the PR-comment scrape)." >&2
        else
