...
`/jds:walkthrough`'s per-finding briefer would write so that
downstream `/jds:fix` and `:walkthrough` can surface a
batch-confirm UI instead of a per-finding interactive loop.

Runs after Phase 5 cross-cutting and before Phase 6 finalize. The
work is downstream of the validator and is Sonnet-driven.
