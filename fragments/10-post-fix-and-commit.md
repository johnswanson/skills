...
#### 9.pre.reconcile — one deep-lane merge agent
...
`model: $high_model` (resolved in Phase 7 step 7.1 from
`JDS_REVIEW_HIGH_MODEL`, default `fable`).

After the agent returns:

1. **Log tokens** via `log-tokens.sh --phase phase_9_reconcile
   --agent-role reconcile --model "$high_model"`. (Match Phase 8 §8.6 step 1 —
...
5. Jump to 9e **no-commit branch**. User-visible error after terminal cleanup:
   ...
   > ERROR: /jds:fix aborted before commit — fix agents
   ...
   > ERROR: /jds:fix — reconcile attempt failed, aborted before
...
### 9a. Phase 9 post-fix review (one deep-lane sub-agent)

Dispatch ONE `Agent` (`subagent_type: general-purpose`,
`model: $high_model`) carrying the §19.9 prompt. Embeds all attempted findings, Phase 8 per-group
...
1. **Log tokens** via `log-tokens.sh --phase phase_9 --agent-role
   post_fix_review --model "$high_model"`.
...
```bash
msg_file="/tmp/jds-fix-msg-$run_id.txt"
reconciled_flag=$(echo "$fix_groups" | jq -r '.[0].id == "FG-RECON"')
{
    ...
    if [[ "$partial_count" -gt 0 || "$reverted_count" -gt 0 ]]; then
        echo "Re-run /jds:fix to address partial and regression findings (retry with revised_fix_proposal context)."
    fi
} > "$msg_file"
```
...
2. **Re-tally `subagent_tokens` and `orchestrator_tokens`** so the
   artifact reflects cumulative spend across `/jds:review` + this
...
6. **(Removed — no push, no publish.)** The upstream pipeline pushed
   the fix commit and updated the PR comment here. This command never
   writes to GitHub — the local commit and artifact are the
   deliverable. Run the publisher in local mode for the audit trail:
...
8. **Surface first failure** (ordered priority):
   ...
   > Re-run /jds:fix to attempt again with revised_fix_proposal context.
   >
   > Commit: `$commit_sha` (local only — not pushed)
...
8. **Surface user-visible degenerate-case error:**
   ...
   - `overlap_inspect` → "Working tree left as-is for manual review.
     ...
     remain `current_state=attempted` — next /jds:fix hard-aborts
   ...
   - `all_regression` → "All $reverted_count fix groups regressed. Tree
     ...
     /jds:fix." (Reconciled all-regression: merge reverted
   ...
   - `revert_failed` → "Per-group revert failed. Tree is in an unknown
     ...
     `current_state` on affected findings and re-run /jds:fix."
