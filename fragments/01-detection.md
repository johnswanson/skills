...
seventh (L7 holistic) joins the fan-out when `--holistic` is set. Each
...
| L2 — structural / blast-radius | `$high_model` | `trivial_mode != true` |
...
| L7 — holistic review | `$high_model` | `holistic_mode == true AND trivial_mode != true` |

Skipped lenses get a one-line note in `trace.md`:
```
Phase 1: L7 skipped (holistic_mode=false)
...
diff; L2 (and L7 under `--holistic`) additionally read surrounding files
...
files under `fragments/lens-prompts/`.
...
the file content is opaque, so the substitution must be explicit, )
...
### 1.2a. Scrape scratch-dir setup

**When `scrape_mode != true`:**

Record one line in `trace.md`:

```
Phase 1 scrape setup skipped — --scrape-pr-comments not set
```

and continue to step 1.2b.

**When `scrape_mode == true`** (Phase 0 step 0.4 found an open PR and
captured `scrape_pr_number`):

Create the scratch directory for scrape output (keeps `$review_dir`
free of transient noise):

```bash
scratch_dir="/tmp/jds-review-$review_id"
mkdir -p "$scratch_dir"
```

No readiness probe is needed — the scrape uses `gh`, which Phase 0
step 0.4 already exercised successfully when it captured
`scrape_pr_number`.
...
**Finally, capture `phase_1_5_start_epoch`** — AFTER the prior-fix
helper completes, so its runtime isn't billed into Phase 1.5's
elapsed (skip when `scrape_mode != true` — Phase 1.5 won't run):

```bash
phase_1_5_start_epoch=$(date +%s)
```

This epoch is what 02-scrape-adapter.md step 1.5.7 subtracts to
compute `phase_1_5_elapsed`.
...
#### L2 — structural / blast-radius (`$high_model`; skipped if `trivial_mode`)

> **Spec data — issue this lens's `Agent` block in the single `#### Dispatch turn` at the end of §1.3, not here.**

Dispatch spec: `model: $high_model` (the Phase 0 step 0.1 value),
...
#### L7 — holistic review (`$high_model`; `holistic_mode` only; skipped if `trivial_mode`)
...
senior reviewer with no checklist. Gated behind `--holistic` because it
costs roughly 1.5–2x an L2 pass. Phase 2 dedup merges overlaps with focused-lens
...
Wait until every internal lens has returned. If `scrape_mode == true`
and `external_candidates` is not yet set, execute
`fragments/02-scrape-adapter.md` (Phase 1.5) NOW and return here once
it has set `external_candidates` — either via its no-input early-skip
(sets `[]`) or via the normalizer at its step 1.5.5. Under
`scrape_mode=false`, `external_candidates` defaults to `[]`.
...
L6, L7, external-pr — stable within source = input order
...
Run steps 3 and 4 in a single `Bash(...)` invocation. If a split is unavoidable, split only after the jq builder runs — write the schema-shaped `$findings_array` (not the pre-builder `$ided`, which the helper rejects as `schema_invalid`) to a mktemp file, then read it back via `--add-findings @<file>` in the next Bash call.
...
Under `--scrape-pr-comments`, `phase_1_elapsed` and Phase 1.5's
elapsed will overlap — Phase 1.5 executes inside Phase 1's join step;
the overlap is the intended observability signal.
