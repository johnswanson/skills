...
One deep-lane (`$high_model`) sub-agent looks across all deep-lane `is_actionable: true`
...
Launch one `Agent` tool-use with `model: $high_model` (the Phase 0
step 0.1 value). No tool access needed;
...
log-tokens.sh \
  ...
  --model "$high_model" --tokens <N or null>
```
...
jq -c '.cross_cutting_groups // .' <<<"$subagent_response_json" \
    > "/tmp/jds-review-ccg-$review_id.json"
artifact-patch.py \
  --path "$artifact_path" \
  --set-json "cross_cutting_groups=@/tmp/jds-review-ccg-$review_id.json"
rm -f "/tmp/jds-review-ccg-$review_id.json"
```
