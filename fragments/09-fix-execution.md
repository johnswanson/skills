...
A non-null `human_confirmation` (set by `/jds:promote`, §27)
...
The `/jds:walkthrough` scope filter is the inverse of this selector; keep the two in sync (see `commands/walkthrough.md` §3).
...
turn. Each agent uses `subagent_type: general-purpose`,
`model: $high_model` (resolved in Phase 7 step 7.1 from
`JDS_REVIEW_HIGH_MODEL`, default `fable`), and receives the full
§19.8 input per-group.
...
<jq output: for each finding_id in $group.finding_ids, emit the finding's
 ...
 /jds:promote --fix-hint, §27) — a labeled block:

   Human-authored fix direction (set via /jds:promote --fix-hint):
   <fix_hint verbatim>
