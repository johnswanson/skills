...
Then resolve the deep-lane model — used for every dispatch the
fragments mark as `model: $high_model` (Phase 8 fix-group agents,
Phase 9a post-fix reviewer, Phase 9.pre reconcile):
...
fired, surface the one-line warning to the user in chat.
...
> `$reviews_root/$repo_slug/`. Run `/jds:review` first.
...
`/tmp/jds-review-invalid-$(date -u +%Y%m%dT%H%M%SZ).json`,
...
- **Stash my changes, run fix, restore** (recommended). Run
  `git stash push --include-untracked -m "pre-jds-review-fix-stash"`
...
if [[ "$head_sha" == "$latest_known_sha" ]]; then
    ...
    if [[ $staleness_rc -eq 0 ]]; then
        ...
        echo "Re-run /jds:review, or check 'git log $latest_known_sha..HEAD' to see what moved." >&2
...
- **(a) Stop — I'll merge `$merge_ref` into `$head_branch` first, then re-run.** Run the stash-pop block
  ...
  trace line, then exit 0 with: `Stopping. Run \`git merge $merge_ref\` (or fast-forward) on \`$head_branch\`, then re-run /jds:fix.`
...
### 7.7. (Removed — no PR eligibility gate)

The upstream pipeline rechecked the PR's open/closed state here
because it published fix results to a PR comment. This command never
publishes; artifacts produced by `/jds:review` are always
`mode=local`. Nothing to do. Step numbering is preserved so
cross-references to 7.8 stay valid.
...
### 7.5.4. Write the auto-rec decisions log (local document)
...
populated are degenerate and shouldn't write a no-op entry).

The decisions log is a local append-only audit document in the review
directory — each `/jds:fix` run appends a fresh entry:

```bash
if (( ${#promoted_ids[@]} > 0 || ${#edited_ids[@]} > 0 )); then
    decisions_body=$(mktemp -t jds-fix-autorec-body.XXXXXX)

    {
        printf '<!-- jds-review-fix-autorec-v1 -->\n'
        ...
        printf '---\n\n'
        printf 'Auto-recommendation acceptance: append-only audit. Each `/jds:fix` run appends a fresh entry. Promoted findings are now `confirmed_mechanical` with `human_confirmation` set; Phase 8 dispatches them next.\n'
    } > "$decisions_body"

    decisions_log_path="$review_dir/fix-autorec-decisions.md"
    cat "$decisions_body" >> "$decisions_log_path"
    printf '\n\n' >> "$decisions_log_path"
    printf 'phase_7_5_decisions_log=%s\n' "$decisions_log_path" \
        >> "$trace_log_path"
...
Tell the user in one line where the decisions log landed. Continue to
7.5.5.
