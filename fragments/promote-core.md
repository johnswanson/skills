...
Shared fragment used by both `/jds:promote` and
`/jds:walkthrough`.
...
**Skipped**: render, local publish trace, user-visible summary — those live in the
top-level command.
...
- Light-lane `confirmed_mechanical` (impact_type ∈ ux/policy/architecture) —
  ...
  bypass. This is the case `/jds:walkthrough`
  exists to address.
- Deep-lane `confirmed_mechanical` below the user's planned threshold — the
  user may run `/jds:fix 70` on a finding scored 55, which
...
hc_tmp=$(mktemp -t jds-promote-hc.XXXXXX)
jq -n \
...
caller's render. A failed patch means the artifact is
